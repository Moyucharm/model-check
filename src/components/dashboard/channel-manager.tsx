...
export function ChannelManager({ onUpdate, className }: ChannelManagerProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const normalizedBaseUrl = formData.baseUrl.trim();
      const normalizedProxy = formData.proxy.trim();
      ...
      if (editingChannel) {
        ...
        const keyList = formData.multiKeys.split(/[,\n]/).map((k: string) => k.trim()).filter(Boolean);
        if (keyList.length === 0) {
          throw new Error("请至少输入一个 API 密钥");
        }
        ...
        if (createData.channel?.id) {
          setShowModal(false);
          setFilterChannels([{ id: createData.channel.id, name: normalizedName }]);
  ...
  const handleDeleteMainKey = async () => {
    ...
    if (channelKeysInfo.length === 0) {
      toast("没有其他密钥可提升为主密钥，无法删除", "error");
      return;
    }
    ...
    try {
      ...
      toast("主密钥已删除，已提升下一个密钥为主密钥", "success");
    } catch (err) {
      toast(err instanceof Error ? err.message : "删除失败", "error");
    } finally {
  ...
  const handleBatchDeleteInvalid = async () => {
    ...
    toast(`已删除 ${deleted} 个无效密钥`, "success");
  ...
  const handleSaveKeysFromTextarea = async () => {
    ...
    try {
      ...
      toast(`已保存 ${keyList.length} 个密钥`, "success");
  ...
  const handleCopyApiKey = async (id: string) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("获取 API 密钥 失败");
      const data = await response.json();
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        payload = { channels: parsed };
      } else if (typeof parsed === "object" && parsed !== null) {
        payload = { ...(parsed as Record<string, unknown>) };
        if (!Array.isArray(payload.channels)) {
          const nestedData = payload.data;
          if (typeof nestedData === "object" && nestedData !== null) {
            const nestedChannels = (nestedData as Record<string, unknown>).channels;
            if (Array.isArray(nestedChannels)) {
              payload.channels = nestedChannels;
            }
          }
        }
      } else {
      ...
      const duplicateInfo = result.duplicates > 0 ? `, 重复 ${result.duplicates}` : "";
      const invalidInfo = result.invalid > 0 ? `, 无效 ${result.invalid}` : "";
      toast(
        `导入成功: 新增 ${result.imported}, 更新 ${result.updated}, 跳过 ${result.skipped}${duplicateInfo}${invalidInfo}${syncInfo}`,
        "success"
      );
    } catch (err) {
                ...
                >
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    ...
                    {(channel._count?.channelKeys ?? 0) > 0 && (
                      <> | <Key className="inline h-3 w-3" /> {(channel._count?.channelKeys ?? 0) + 1} 个密钥</>
                    )}
                    {" "}| 密钥: {channel.apiKey}
                  </div>
                  {/* Sync status message */}
                ...
                {editingChannel && keyViewMode === "list" && (
                  <div className="space-y-2">
                    <p className="text-xs text-muted-foreground">
                      共 {channelKeysInfo.length + (maskedApiKey ? 1 : 0)} 个密钥
                    </p>
                        ...
                        <div className="flex items-center gap-2 px-2.5 py-1.5 bg-blue-500/5 overflow-hidden">
                          {editingKeyTarget === "main" ? (
                            ...
                            <>
                              ...
                              <span className="text-xs text-blue-500 shrink-0">主密钥</span>
                              <button
                                type="button"
                              ...
                              <button
                                ...
                                title={channelKeysInfo.length === 0 ? "没有其他密钥可提升，无法删除" : "删除主密钥，下一个密钥将提升为主密钥"}
                              >
                                {deletingMainKey ? (
                ...
                {(!editingChannel || (editingChannel && keyViewMode === "edit")) && (
                  <div className="space-y-2">
                    <textarea
                      ...
                      placeholder={editingChannel ? "修改后保存将覆盖所有密钥，一行一个" : "一行一个密钥，第一个为主密钥"}
                    />
                    {editingChannel && (
                      <button
                        type="button"
